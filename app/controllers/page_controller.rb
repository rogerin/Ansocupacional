class PageController < ApplicationController
  def index
    ...
    @acessos = scope.includes(:empresa).order("log_empresas.created_at DESC").select("log_empresas.empresa_id, log_empresas.id, log_empresas.mensagem, log_empresas.created_at" ).group("log_empresas.empresa_id").limit(20)
